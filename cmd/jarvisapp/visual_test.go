...
func TestVisualArgs_Isolated(t *testing.T) {
	...
	for _, want := range []string{"127.0.0.1:9999", "jobs_visualcheck", "tickets_visualcheck", "--models-file=", "--watch-dir=", "--out-dir=", "--agent-dev=false", "--deploy=false", "--mail-config=", "--mail-collection=emails_check"} {
...
func TestSmokeArgs_Isolated(t *testing.T) {
	...
	for _, want := range []string{"127.0.0.1:9999", "jobs_deploycheck", "tickets_deploycheck", "--models-file=", "--mail-config=", "--mail-collection=emails_check", "--watch-dir="} {
		if !strings.Contains(got, want) {
			t.Errorf("args lack %q: %s", want, got)
		}
	}
